function Segment2D_fixDeg(tFun,segSet)

%--------------------------------------------------------------------------
% OBJECT:
%--------------------------------------------------------------------------
% This demo shows the behaviour of the error at fix degree when the number 
% of segments increases when computing least squares on whitney 1-forms
% calculated on segments on a triangles. Modify the parameter do_plot to
...
h = h+1;
% STATISTICS.
fprintf('\n \t -> M: %3d errLS: %1.3e', M, errLS(h-1)); 
end
fprintf('\n \t -> errINT: %1.3e', errINT(end)); 
if do_plot
    clear_figure(2);
    fig2 = figure(2);
    semilogy(Mseg,errLS,'o-b')
    hold on;
    semilogy(Mseg,errINT,'o-r')
